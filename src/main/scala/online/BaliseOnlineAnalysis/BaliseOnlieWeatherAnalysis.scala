...
import utils.{MakeBaliseKPI, MakeOnlineWeather, MakeRedisConn, ManagerOffset}
...
object BaliseOnlieWeatherAnalysis {
    ...
    dstream.foreachRDD(rdd => {
      ...
      if (!rdd.isEmpty()) {
        ...
        filted.map(arr => {
          ...
          val wea = arr(53)
          //获取天气等级
          val weaLevel = MakeOnlineWeather.getWeaLevel(wea)
          //获取指标哦
