...
object SingalTimeOnlineAnalysis {
  def main(args: Array[String]): Unit = {
    ...
    //读取列出出厂时间的数据ssc
    val signalTimeSource: RDD[String] = ssc.sparkContext.textFile("F:\\项目\\高铁项目\\信号机生产日期.txt")
    ...
    val signalTimeBro: Broadcast[collection.Map[String, String]] = ssc.sparkContext.broadcast(signalTimeMap)
    ...
    dstream.foreachRDD(rdd => {
      ...
      if (!rdd.isEmpty()) {
        ...
        filted.map(arr => {
          ...
          val signalid = arr(27)
          //获取信号机出厂时间
          val signalTime = signalTimeBro.value.getOrElse(signalid,signalid)
          //获取指标哦
          val signalKpi = MakeSignalKPI.getSignalKPI(arr)
          (signalTime, signalKpi)
        }).reduceByKey {
