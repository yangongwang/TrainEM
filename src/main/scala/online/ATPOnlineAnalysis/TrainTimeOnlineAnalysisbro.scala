...
object TrainTimeOnlineAnalysisbro {
  def main(args: Array[String]): Unit = {
    ...
    //读取列出出厂时间的数据
    val trainTimeSource: RDD[String] = ssc.sparkContext.textFile("F:\\项目\\高铁项目\\列车出厂时间数据.txt")
    ...
    val trainTimeBro: Broadcast[collection.Map[String, String]] = ssc.sparkContext.broadcast(trainTimeMap)
    ...
    dstream.foreachRDD(rdd => {
      ...
      if (!rdd.isEmpty()) {
        ...
        filted.map(arr => {
          ...
          val trainID = arr(1)
          //获取列车出厂时间
          val trainTime = trainTimeBro.value.getOrElse(trainID,trainID)
          //获取指标哦
